...
<script type="text/javascript">
	...
	for($i=0; $i<count($rst); $i++) {
		...
		if($mobile == 1) {
			$pop_x_position = $pop_x_position - 150;
			if($i%2 == 0 && $i > 0) {
				$pop_x_position = 0;
				$pop_y_position = $pop_y_position + $rst[$i-2]['pop_x_position'] + 60;
			}
		}
		...
		if ($pop_type=="T") {
			...
			//이미지 클릭시 이동할 페이지
			if($pop_link_url != "") {
				$pop_str="<a href=\"javascript:move_page('${pop_link_url}')\"><img src='${url}' border='0' class='pop_img_" . $pop_idx . "'></a>";
			} else {
				$pop_str="<img src='${url}' border='0' class='pop_img_" . $pop_idx . "'>";		
			}
		}
	...
	<div id="divpop_<?=$pop_idx?>" class="popup-layer" style="width:<?=$pop_width?>px; height:<?=$pop_height?>px; position:absolute; left:<?=$pop_x_position?>px; top:<?=$pop_y_position?>px; z-index:10000; visibility: hidden; overflow:<?=$scrollbars?>;">
  <form name="Pop_form_<?=$pop_idx?>">
    ...
    <div style="background-color:#000; padding-right:10px; font-size:9pt; color:#FFFFFF; text-align:right; height:20px;">
      <input name="chkbox" type="checkbox" value="1" /> 이 창을 <?=$cookie_text?> 열지 않습니다.
      <a href="javascript:closeWin_<?=$pop_idx?>();"><font color="#754d21"><B>[닫기]</B></font></a> 
    </div> 
  </form>     
  </div> 
	<style type="text/css">
    	.pop_img_<?=$pop_idx?> { width:<?=$pop_width?>px; height:<?=$css_img_height?>px;}
   </style>
	<script type="text/javascript">
