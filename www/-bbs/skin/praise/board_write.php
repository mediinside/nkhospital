...
<fieldset>
	...
	<table width="100%" class="writeType">
		...
		<tbody>			
			<tr>
				<th scope="row">제 목</th>
				<td colspan="3"><input type="text" class="txtInput" style="width:578px;" id="jb_title" name="jb_title" /></td>
			</tr>
			<tr>
				<th scope="row">칭찬대상</th>
				<td><input type="text" class="txtInput" style="width:228px;" id="jb_praise_name" name="jb_praise_name" /></td>
				<th scope="row">소속</th>
				<td><? echo $treat_select = $C_Func -> makeSelect('jb_treat', $GP -> CLINIC_TYPE_NEW2, $jb_treat, ' class="selType" ', '::선택::');?></td>
			</tr>
			<tr>
				<th scope="row">작성자</th>
				<td colspan="3"><input type="text" class="txtInput" style="width:228px;" id="jb_name" value="<?=$check_name?>" name="jb_name" /></td>
			</tr>
			<?php
				...
				<tr>
					...
					<td colspan="3"><input type="text" id="jb_password" value="" name="jb_password" style="width:228px;" /></td>
				</tr>
				<?php
				} else {
      ...
      <!--tr>
        ...
        <td colspan="3">	
						<span class="chkBox">					
						<input type="checkbox" value="Y" id="jb_secret_check" name="jb_secret_check" /><label>비밀글</label>
          	<?
						if(isset($check_id) && $check_level>=9){
							echo "<input type=\"checkbox\" value=\"Y\" id=\"jb_notice_check\" name=\"jb_notice_check\"><label>공지</label>";
						}
						?>
						</span>
						</td>
			...
			<tr>
				...
				<td colspan="3"><input type="text" class="txtInput" style="width:578px;" id="jb_homepage" name="jb_homepage" /></td>
			</tr>
      <tr>
      ...
      <tr>
        ...
        <td colspan="3"><img src="<?=$GP -> IMG_PATH?>/zmSpamFree/zmSpamFree.php?zsfimg=<?php echo time();?>" id="zsfImg" alt="아래 새로고침을 클릭해 주세요." />
...
<script type="text/javascript">
	...
	$('#img_submit').click(function(){
		...
		if($('#jb_praise_name').val() == '')	{
			alert('칭찬대상을 입력하세요');
			$('#jb_praise_name').focus();
			return false;
		}		
