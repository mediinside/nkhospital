...
<form name="frm_Board" id="frm_Board" action="<?=$get_par?>" method="post" enctype="multipart/form-data">
  <input type="hidden" name="jb_password" value="<?=$input_passd;?>">
	<input type="hidden" id="jb_secret_check" name="jb_secret_check" value="Y" />
<fieldset>
	...
	<table width="100%" class="writeType">
		...
		<tbody>
			...
			<tr>
				<th scope="row">구분</th>
				<td><? echo $treat_select = $C_Func -> makeSelect_Normal('jb_treat', $GP -> CLINIC_TYPE5, $jb_treat, ' style="border:1px solide #ddd; height:27px; line-height:27px;" ', '::선택::');?></td>
			</tr>
			<tr>
				<th scope="row">작성자</th>
			...
			<tr>
				<th scope="row">연락처</th>
				<td><input type="text" class="txtInput" style="width:228px;" id="jb_mobile" value="<?=$jb_mobile;?>" name="jb_mobile" /></td>
			</tr>
			<tr>
        <th scope="row">이메일</th>
...
<script type="text/javascript">
	...
	$('#img_submit').click(function(){
		...
		if($('#jb_treat option:selected').val() == '') {
			alert('구분을 선택해주세요');
			return false;
		}
