	...
	<div class="searchCont btnWrap">
		...
		<?
			...
			if($check_level >= $db_config_data['jba_write_level']) {
				echo "<a class='btn bgc-red' style='padding:0 15px;color:#fff !important;border:none;' href=\"#\" onclick=\"javascript:location.href='${index_page}?jb_code=${jb_code}&jb_mode=twrite'\" title='글쓰기' ><span style='padding:0;'>글쓰기</span></a>";
			} else {
				echo "<a class='btn bgc-red' style='padding:0 15px;color:#fff !important;border:none;' href=\"javascript:alert('로그인후 글쓰기가 가능합니다.');\" title='글쓰기' ><span style='padding:0;'>글쓰기</span></a>";
			}
		 ?>
		 </div>
	</div>
