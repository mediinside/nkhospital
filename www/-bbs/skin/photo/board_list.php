	...
	<div class="searchCont btnWrap">
		...
		<?
			if($_GET['search_key'] && $_GET['search_keyword']) {
				echo "<a class='btn bgc-red' href=\"javascript:;\" onclick=\"javascript:location.href='${index_page}?jb_code=${jb_code}'\" title='목록보기'  style='padding:0 15px;color:#fff !important;border:none;'><span style='padding-right:0;'>목록보기</span></a>&nbsp;&nbsp;";
			}

			//쓰기권한
			if($check_level >= $db_config_data['jba_write_level']) {
				echo "<a class='btn bgc-red' href=\"#\" onclick=\"javascript:location.href='${index_page}?jb_code=${jb_code}&jb_mode=twrite'\" title='글쓰기'  style='padding:0 15px;color:#fff !important;border:none;'><span style='padding-right:0;'>글쓰기</span></a>";
			} else {
				echo "<a class='btn bgc-red' href=\"javascript:alert('로그인후 글쓰기가 가능합니다.');\" title='글쓰기'  style='padding:0 15px;color:#fff !important;border:none;'><span style='padding-right:0;'>글쓰기</span></a>";
			}
		 ?>
		 </div>
	</div>
