...
if($data_list_cnt > 0) {
	for ($i=0; $i<$data_list_cnt; $i++ ) {		
			...
			if(strlen($jb_nick_name) > 6) {
				$jb_nick_name 				=  $C_Func->blindInfo($jb_nick_name, 6);
			}else{
			...
			$jb_praise_name	= $data_list[$i]['jb_praise_name'];
			$jb_step				= $data_list[$i]['jb_step'];
			$jb_title 				= $C_Func->strcut_utf8($data_list[$i]['jb_title'], 52,true, "...");		//제목 (길이, HTML TAG제한여부 처리)
			...
			//공지글여부
			if($jb_order < 100) {
				$jb_title = "<strong>[공지사항] " . $jb_title. "</strong>";		
			
			echo ("
				<tr " . $tr_class. ">\n
					<td >" . $num_idx--. "</td>\n		
					<td >" . $GP -> CLINIC_TYPE_NEW2[$jb_treat] . "</td>\n
					<td >${jb_praise_name}</td>\n
					<td class='txtL '>${jb_title}</td>\n
					<td >${jb_reg_date}</td>\n
				</tr>\n
				");
			} else {
