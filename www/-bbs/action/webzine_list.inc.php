...
if($data_list_cnt > 0) {
	for ($i=0; $i<$data_list_cnt; $i++ ) {		
			...
			$jb_title_ori 	= $C_Func->strcut_utf8($data_list[$i]['jb_title'], 100, "...");	//제목 (길이, HTML TAG제한여부 처리)
			$jb_title 			= $C_Func->strcut_utf8($data_list[$i]['jb_title'], 100, "...");	//제목 (길이, HTML TAG제한여부 처리)
...
?>
		<li><a href="<?=$get_par?>" class="panel">
            ...
            <dl class="post">
                <dt class="header">
                    <small class="category"><?=$GP ->WEBZINE_TYPE[$jb_type]?></small>
                    <strong class="subject"><?=$jb_title ?></strong>
                </dt>
                <dd class="contents"><?=$content?></dd>
                <dd class="writer">뉴고려병원</dd>
                <dd class="views"><i class="ip-icon-views"></i><span class="text"><?=$jb_see?></span></dd>
                <dd class="date"><i class="ip-icon-date"></i><span class="text"><?=$jb_reg_date?></span></dd>
            </dl>
        </a></li>
<?
			$dummy++;
