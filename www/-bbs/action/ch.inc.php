...
if($data_list_cnt > 0) {
	for ($i=0; $i<$data_list_cnt; $i++ ) {		
			...
			$jb_name 					= str_replace(" ","", $data_list[$i]['jb_name']);
			$jb_nick_name 		= $data_list[$i]['jb_nick_name'];
			...
			$jb_title 				= $C_Func->strcut_utf8($data_list[$i]['jb_title'], 52, "...");	//제목 (길이, HTML TAG제한여부 처리)
			...
			if($check_level < 9 && $data_list[$i]['jb_secret_check'] == "Y")
			{
				...
				if($jb_mb_id == "") {
					...
					if($check_id != $jb_mb_id) {
						...
						if (trim($check_id) == trim($rst['jb_mb_id'])) {
							$get_par = "${index_page}?jb_code=${jb_code}&jb_idx=${jb_idx}";
			...
			{	
				...
				$img_src = "<img src='" . $code_file. "' alt='" . $jb_title_ori . "' width='162' height='108'>";	
			}else{
				$img_src = "<img src='/images/main/img_tmp_mov2.gif' width='162' height='108'>";	
			}
...
?>
				<li>
					<a href="<?=$get_par?>">
						<span class="thumb">
							<?=$img_src?>
							<img src="/images/main/img_mov_play2.png" alt="" class="play" />
						</span>
						<strong class="subject">
							<?=$jb_title?>
						</strong>
						<span class="info">
							게시자 : <?=$jb_name?> <span>|</span> 조회수 : <?=$jb_see?>회 <span>|</span> <?=$jb_reg_date?>
						</span>
                        <? if($jb_code == "240"){
								if($jb_type == "A"){
									$ttxt = "전문센터";
									$trtxt = $GP->NEW_MOBILE_CENTER_ALL[$jb_treat];
								}else if($jb_type == "B"){
									$ttxt = "진료과";
									$trtxt = $GP->NEW_MOBILE_CENTER_ALL[$jb_treat];									
								}else if($jb_type == "C"){
									$ttxt = "특수클리닉";
									$trtxt = $GP->NEW_MOBILE_SPECIAL[$jb_treat];									
								}
							?>
                            <br /><br />
                            <span class="info">
                                <?=$ttxt?> <span>|</span> 구분 : <?=$trtxt?>
                            </span>
                        <? } ?>                        
					</a>
				</li>
<?
	...
	echo "<li style='text-align:center; padding-top:20px;'>등록된 데이터가 없습니다.</li>";
}
?>