...
if($data_list_cnt > 0) {
	for ($i=0; $i<$data_list_cnt; $i++ ) {		
            ...
            if($jb_qcode == "news") $get_par.= "&qcode=news";
            ...
            //echo "echo:" . $jb_title . "<br>" ;

/*			//공지글여부
			if($jb_order < 100) {
				$jb_no =" <td class='num notice'></td>";
			}
			
*/			
				if($jb_order < 100) {
					$cls =" class ='notice'";
					$jb_no ="<strong class='notice'>공지</strong>";
				}else{
					$cls ="";
					$jb_no ="" . $num_idx--. "";
				}
				...
				<tr$cls>
					<td>${jb_no}</td>
					<td class='subject'><a href='" . $get_par . "'>${jb_title}</a></td>
					<td>${jb_reg_date}</td>
				</tr>
				");
			$dummy++;
