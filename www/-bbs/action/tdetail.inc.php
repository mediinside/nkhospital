...
if($board_data) {
	...
	if($check_level < 9 && $jb_secret_check == "Y" && $_SESSION['ssecret_chk_key'] != $jb_idx) {		
		//비회원이 작성한 비밀글일 경우
		if($jb_mb_id == "") {
			 //비밀번호 입력 체크
...
$vd_link2= str_replace("watch?v=", "embed/", $vd_link1);
$vd_link2= str_replace("&t=", "?start=", $vd_link2);
	...
	//이전글
	if($be_idx != '') {
		$get_par1 = "${index_page}?jb_code=${jb_code}&jb_idx=${be_idx}&jb_group=${jb_group}&jb_step=${jb_step}&jb_depth=${jb_depth}";
		$get_par1.= "&${search_key}&search_keyword=$search_keyword&page=${page}";
	}	
	
	//다음글
	if($af_idx != '') {
		$get_par2 = "${index_page}?jb_code=${jb_code}&jb_idx=${af_idx}&jb_group=${jb_group}&jb_step=${jb_step}&jb_depth=${jb_depth}";
		$get_par2.= "&${search_key}&search_keyword=$search_keyword&page=${page}";
	}
