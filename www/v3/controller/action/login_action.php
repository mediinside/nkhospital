...
include_once($GP->CLS."class.member.php");
	...
	if($mode == "LOGIN"){
		if(!ereg($server, $refer)){
			$C_Func->put_msg_and_go("올바른 경로로 접근 바랍니다", "/v3/");
			exit();
		}	
	}
	switch($mode){
		...
		case "LOGIN":
			...
			if($rst['mb_password'] ==  md5(trim($m_pwd))) {
				...
				if ($re_url != "") {
					...
					$C_Func ->	go_url($GP -> SERVICE_DOMAIN."/v3/");
				}
			} else {
		...
		case 'naver':
			...
			exit;				
				...
				if ($re_url != "") {
					$C_Func ->	go_replace_opener2("http://nkhospital.net/v3".$re_url);
				} else {
					if($u == "M"){
				...
				?>
					<script>
						opener.top.location.href = 'http://nkhospital.net/v3/';
						setTimeout(function(){
								self.close();
						},300);
					</script>
				...
				<?php
						...
						$C_Func ->	go_replace_opener2("http://nkhospital.net/v3/");
					}
				}            
