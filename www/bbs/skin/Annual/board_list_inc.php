...
if($data_list_cnt > 0) {
	for ($i=0; $i<$data_list_cnt; $i++ ) {		
			...
			if($file_cnt > 0)
			{	
                $code_file = "/bbs/files/jb_${jb_code}/${ex_jb_file_code[0]}";
                $img_src = "<img src='" . $code_file. "' alt='" . $jb_title_ori . "' >";	

                $code_file2 = $GP->UP_IMG_SMARTEDITOR. "jb_${jb_code}/${ex_jb_file_code[1]}";							
                $code_name ="/bbs/download.php?downview=1&file=" . $code_file2 . "&name=" . $ex_jb_file_name[1] ;
				
            }

            //echo "echo:" . $jb_file_code . "<br>" ;
