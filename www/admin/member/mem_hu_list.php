<?php
	...
	$data = "";
	$data = $C_Member->Mem_Hu_List(array_merge($_GET,$_POST,$args));
...
<div class="Wrap"><!--// 전체를 감싸는 Wrap -->
		...
		<div class="boxContentBody">
            ...
            <option value="mb_id" <? if($_GET['search_key'] == "mb_id"){ echo "selected";}?> >아이디</option>
					...
					<span><input type="button" id="email_btn" value="휴먼계정 선택 메일발송" /></span>
					<!-- span><input type="button" id="email_all_btn" value="전체 메일" /></span -->
				</li>
			</ul>
			</form>
			</div>
						...
						<tbody>
							<?
								...
								for ($i = 0 ; $i < $data_list_cnt ; $i++) {
									...
									$mb_lastlogin_date = $data_list[$i]['mb_lastlogin_date'];	
...
<script type="text/javascript">
	...
	$(document).ready(function(){														 
		...
		$('#email_btn').click(function(){			
			if(!confirm("휴먼메일을 발송하시겠습니까?")) return;
			...
			$.ajax({
				...
				data: "mode=HUEMAIL_SEND&arr_idx=" + num,
				dataType: "text",
				beforeSend : function(){
