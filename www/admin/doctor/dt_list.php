<?php
	...
	$cn1_select = $C_Func -> makeSelect_Normal('dr_clinic2', $GP -> NEW_MOBILE_CENTER_ALL, $_GET['dr_clinic2'], '', '::선택::');
?>
<body>
<div class="Wrap"><!--// 전체를 감싸는 Wrap -->
		...
		<div class="btnWrap">
			...
			<button onClick="layerPop('ifm_reg','./dr_reg.php', '100%', 700)"; class="btnSearch btnRight">의료진 등록</button>
		</div>
						...
						<tbody>
							...
							<?
								...
								for ($i = 0 ; $i < $data_list_cnt ; $i++) {
									...
									$book_btn = $C_Button -> getButtonDesign('type2','관리',0,"layerPop('ifm_reg','./book_list.php?dr_idx=" . $dr_idx. "', '100%', 700)", 50,'');									
									$edit_btn = $C_Button -> getButtonDesign('type2','수정',0,"layerPop('ifm_reg','./dr_edit.php?dr_idx=" . $dr_idx. "', '100%', 700)", 50,'');	
											...
											<td>
												<?
													...
													foreach ($cn_arr3 as $key => $val) {
														$str .= $GP -> CLINIC_TYPE3[$val] . ",";
													}
													echo rtrim($str , ",");
												?>
											</td>
											<td><?=($dr_qa_show == "Y") ? "공개":"비공개";?></td>
											<td><?=$dr_regdate?></td>
											<td><?=$book_btn?></td>									
											<td><?=$edit_btn?></td>
										</tr>
