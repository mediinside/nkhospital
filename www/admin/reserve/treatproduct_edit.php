<?php
	...
	$rst1 = $C_Reserve -> TrestSection_All($args);
	...
	$args = "";
	$args['pd_idx'] 	= $_GET['pd_idx'];
	$args['ts_idx'] 	= $_GET['ts_idx'];
	$args['hp_idx'] 	= $_SESSION['adminhpidx'];
	$rst = $C_Reserve ->Treat_PD_Info($args);
	
	if($rst) {
		extract($rst);
	}
	
	$sel_treat = $C_Func ->makeSelect("ts_idx", $rst1, $ts_idx, "class='select_type1'", "::: 선택 :::");	
...
<div class="Wrap_layer"><!--// 전체를 감싸는 Wrap -->
	<div class="boxContent_layer">
		<div class="boxContentHeader">
			<span class="boxTopNav"><strong>진료 상품 수정</strong></span>
		</div>
		<form name="base_form" id="base_form" method="POST" action="?">
		<input type="hidden" name="mode" id="mode" value="TREATPRODUCT_MODI" />
		<input type="hidden" name="pd_idx" id="pd_idx" value="<?=$_GET['pd_idx']?>" />
				...
				<table class="table table-bordered">
					<tbody>
						...
						<tr>
							<th ><span>*</span>상품명</th>
							<td>
								<input type="text" class="input_text" size="30" name="pd_name" id="pd_name" value="<?=$pd_name?>"  />
							</td>
						</tr>
						<tr>
