...
(function($) {
    ...
    $.fn.colorselect = function(option) {        
        ...
        var aitem = '<a href="#" id="{1}-{0}" key="{2}" hidefocus="on"><em><span style="background:#{0};border:solid 1px #{3}" unselectable="on">&#160;</span><label style="display:inline-block;">{4}</label></em></a>';

				var colors = [
        // "F7B281", "EA878B", "FDFC77", "8DADE1", "9CDA95", "CF9CB5"
		    ];
        var title = ["인터넷예약","전화예약","당일접수","진료대기","진료중","진료완료","예약부도"];
				var borders = [];				
				/*
        ...
        var blanka = '<a href="#" id="{1}-{0}" key="{2}" hidefocus="on"><em><span style=" text-align:center; display:inline-block; width:46px;padding-top:2px; height:11px;border:solid 1px #8B7B8B;" unselectable="on">none</span></em></a>'
        var blank = [];
        blank.push("ffffff");
        blank.push(id);
        blank.push(-1);
        ...
        $("#div" + id + "  a").click(function(e) {
            ...
            if (options.hiddenid != null && options.hiddenid != "") {
                $("#" + options.hiddenid).val(key);
								$('#show_status').html("<div style='display:inline-block; vertical-align:top;'> " + title[key] + "</div>");
            }
        ...
        if (options.index != -1) {
            leftdiv.css("border", " solid 1px " + "#" + borders[options.index]);
						show_html = "<div id='show_status' style='display:inline-block; vertical-align:top;'> " + title[options.index] + "</div>";
        }
        else {
            leftdiv.css("border", "solid 1px #8B7B8B");
						show_html = "<div id='show_status' style='display:inline-block; vertical-align:top;'>none</div>";
        }
        if (options.hiddenid != null && options.hiddenid != "") {
