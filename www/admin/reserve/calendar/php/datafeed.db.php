...
function addCalendar($st, $et, $sub, $ade){
  ...
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "insert into `hp_memreserve` (`mb_name`, `mr_s_time`, `mr_e_time`, `mr_alldayevent`) values ('"
...
function addDetailedCalendar($st, $et, $sub, $ade, $dscr, $loc, $color, $tz){
  ...
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "insert into `hp_memreserve` (`mb_name`, `mr_s_time`, `mr_e_time`, `mr_alldayevent`, `mr_memo`, `mr_product`, `mr_Color`) values ('"
      .mysql_real_escape_string($sub)."', '"
      .php2MySqlTime(js2PhpTime($st))."', '"
      .php2MySqlTime(js2PhpTime($et))."', '"
      .mysql_real_escape_string($ade)."', '"
  ...
  try{
    ...
    $sql = "select * from `jqcalendar` where `starttime` between '"
      .php2MySqlTime($sd)."' and '". php2MySqlTime($ed)."'";
		*/
		$sql = "
			select * from hp_memreserve where mr_s_time between '" . php2MySqlTime($sd) . "' and '". php2MySqlTime($ed)."'
		";		
    ...
    while ($row = mysql_fetch_object($handle)) {
      ...
      $ret['events'][] = array(
        ...
        $row->mr_idx,
        $row->mb_name,
        php2JsTime(mySql2PhpTime($row->mr_s_time)),
        php2JsTime(mySql2PhpTime($row->mr_e_time)),
        $row->mr_alldayevent,
        0, //more than one day event
        //$row->InstanceType,
        0,//Recurring event,
...
function updateCalendar($id, $st, $et){
  ...
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "update `hp_memreserve` set"
      . " `mr_s_time`='" . php2MySqlTime(js2PhpTime($st)) . "', "
      . " `mr_e_time`='" . php2MySqlTime(js2PhpTime($et)) . "' "
      . "where `mr_idx`=" . $id;
    //echo $sql;
		if(mysql_query($sql)==false){
      $ret['IsSuccess'] = false;
...
function updateDetailedCalendar($id, $st, $et, $sub, $ade, $dscr, $loc, $color, $tz){
  ...
  try{
    ...
    $sql = "update `hp_memreserve` set"
      . " `mr_s_time`='" . php2MySqlTime(js2PhpTime($st)) . "', "
      . " `mr_e_time`='" . php2MySqlTime(js2PhpTime($et)) . "', "
      . " `mb_name`='" . mysql_real_escape_string($sub) . "', "
      . " `mr_alldayevent`='" . mysql_real_escape_string($ade) . "', "
      . " `mr_memo`='" . mysql_real_escape_string($dscr) . "', "
      . " `mr_product`='" . mysql_real_escape_string($loc) . "', "
      . " `mr_Color`='" . mysql_real_escape_string($color) . "' "
      . "where `mr_idx`=" . $id;
    //echo $sql;
		if(mysql_query($sql)==false){
      $ret['IsSuccess'] = false;
