<?php
	...
	$data = "";
	$data = $C_Reserve->TreatProduct_List(array_merge($_GET,$_POST,$args));
...
<div class="Wrap"><!--// 전체를 감싸는 Wrap -->
			...
			<ul>				
				...
				<li>
					...
					<select name="search_key" id="search_key">
						...
						<option value="PD.pd_name" <? if($_GET['search_key'] == "PD.pd_name"){ echo "selected";}?> >과명</option>						
					</select>
					</span>
		...
		<button onClick="layerPop('ifm_reg','./treatproduct_reg.php', 500, 200)"; class="btnSearch ">상품 등록</button>
		</div>
						...
						<tbody>
							 <?
								...
								for ($i = 0 ; $i < $data_list_cnt ; $i++) {
									...
									$pd_idx 	= $data_list[$i]['pd_idx'];
									$hp_idx 	= $data_list[$i]['hp_idx'];
									$ts_name	= $data_list[$i]['ts_name'];	
									$pd_name	= $data_list[$i]['pd_name'];	
									$pd_capanum	= $data_list[$i]['pd_capanum'];	
									$pd_regdate	= date("Y.m.d", strtotime($data_list[$i]['pd_regdate']));
									
									$edit_btn = $C_Button -> getButtonDesign('type2','수정',0,"layerPop('ifm_reg','./treatproduct_edit.php?ts_idx=" . $ts_idx. "&pd_idx=" . $pd_idx. "', 500, 200)", 50,'');	
									$edit_btn .= $C_Button -> getButtonDesign('type2','삭제',0,"pd_delete('" . $pd_idx. "')", 50,'');							
								?>
								<tr>
...
<script type="text/javascript">
	...
	function pd_delete(pd_idx)
	{
		...
		$.ajax({
			...
			data: "mode=TREATPRODUCT_DEL&pd_idx=" + pd_idx,
			dataType: "text",
