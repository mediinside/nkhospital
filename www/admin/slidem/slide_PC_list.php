...
<div class="Wrap"><!--// 전체를 감싸는 Wrap -->
		...
		<button onClick="layerPop('ifm_reg','./slide_PC_reg.php', '100%', 650)"; class="btnSearch ">슬라이드 등록</button>
		</div>
							...
							<?
								...
								if($data_list_cnt > 0 ) {
									for ($i = 0 ; $i < $data_list_cnt ; $i++) {
										...
										$ts_gubun 		= $data_list[$i]['ts_gubun'];
										$ts_regdate		= $data_list[$i]['ts_regdate'];																			
										$ts_gubun_txt	= ($ts_gubun=="T") ? "상단" : "메인" ;
										...
										$edit_btn = $C_Button -> getButtonDesign('type2','수정',0,"layerPop('ifm_reg','./slide_PC_edit.php?ts_idx=" . $ts_idx. "', '100%', 650)", 50,'');	
