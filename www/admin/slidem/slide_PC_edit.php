...
<div class="Wrap_layer"><!--// 전체를 감싸는 Wrap -->
	<div class="boxContent_layer">
		...
		<input type="hidden" name="ts_type" id="ts_type" value="PC" />
		<input type="hidden" name="ts_gubun" id="ts_gubun" value="M" />
				...
				<div class="layerTable">
					<table class="table table-bordered">
						<tbody>
							...
							<tr>
								...
								<td>
									<input type="text" class="input_text" size="70" name="ts_link" id="ts_link" value="<?=$ts_link?>" />
                                      <p class="colorIdt">※주소 입력시 http://allspine.co.kr 를 뺀 나머지 부분만 입력해 주세요.</p>
								</td>
							</tr>
							<!-- tr>
								<th><span>*</span>내용</th>
							...
							<!--tr>
								<th><span>*</span>슬라이드구분</th>
								<td>
									<label>
										<input type="radio" name="ts_gubun" value="T" <? if($ts_gubun == "T"){ echo "checked";}?>/> 상단
									</label>
									<label>
										<input type="radio" name="ts_gubun" value="M" <? if($ts_gubun == "M"){ echo "checked";}?>/> 메인
									</label>
								</td>
							</tr-->                           							
						<tr>
							<th><span>*</span>모바일 이미지(슬라이드)</th>
							<td>
								...
								<?
									if($ts_img != "") {
										echo  "<br>" . $ts_img;
								?>
									<a href="#" onClick="img_del('<?=$ts_img;?>','<?=$_GET['ts_idx']?>','M')">(X)</a>
								<? } ?>
							</td>
						</tr>
						</tbody>
