...
CLASS Main extends Dbconn
{
	...
	function Main_Slide_List($args = '') {
		$qry = "
			select * from tblwebzineslide where twzs_show='Y' and twzs_del_flag='N' order by twzs_desc desc
		";
		$rst =  $this -> DB -> execSqlList($qry);
		return $rst;
	}
	...
	// param
	function Cate_List() {		
		$qry = " select * from tblCategory where ct_show ='Y' and ct_del_flag ='N' order by ct_desc asc";
		$rst = $this -> DB -> execSqlList($qry);
		return $rst;
	}
