...
CLASS Dbconn
{
	...
	public function db_connect()
	{
		if (!is_resource($this -> connect)) {
			...
			$this -> connect = @mysql_connect( $this->host, $this->dbuser, $this->dbpass ,true) or die( "현재 접속이 원활하지 않습니다1. 잠시 후 다시 시도해 주시기 바랍니다. " ); 
			@mysql_query("SET NAMES utf8");
			@mysql_query("set character_set_results=utf-8");
			if( !@@mysql_select_db( $this->dbname, $this -> connect ) ) {
				//echo "$this->dbname select ERROR1 " ;
				echo "현재 접속이 원활하지 않습니다11. 잠시 후 다시 시도해 주시기 바랍니다. " ;
			}
		}
	}	
