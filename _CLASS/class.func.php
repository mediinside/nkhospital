...
*/
	...
	function makeCheckbox_Normal () {
		...
		if (is_array($value_arr)) {
			foreach ($value_arr as $k => $v) {
				...
				$str .= "<div style='display:inline-block; line-height:20px; width:" . $width ."px;'><input type=\"checkbox\" name=\"$name\" value=\"$k\" $checked_str $etc> $v</div>";
			}
		}

		return $str;
	}
	function makeCheckbox_Normal_1 () {
		$args = @func_get_args ();
		$paraArr = array(
					'value_arr'
					,'name'
		...
		if (is_array($value_arr)) {
			foreach ($value_arr as $k => $v) {
				...
				$str .= "<div style='display:inline-block; line-height:20px; width:" . $width ."px;'><input type=\"checkbox\" name=\"$name\" value=\"$k\" $checked_str $etc></div>";
			}
		}
  ...
  // 상태 select array 받아오기
	...
	function request_add_day2($start_date, $add_day)
	{
		$date = date("Ymd", strtotime($start_date."+". $add_day." days"));
		return $date;
	}
