...
CLASS Analytics extends Dbconn
{
	...
	// auth  : JH 2013-09-16 월요일
	// param
	function Analytics_Video_List ($args = '') {		
		...
		if (($s_date && $e_date) && ($s_date < $e_date)) {
			if ($addQry)
			$addQry .= " AND ";
			$addQry .= " DATE_FORMAT(vd_regdate,'%Y-%m-%d') between '$s_date' AND '$e_date'";
		}

		$qry = "
			SELECT 
				vd_dr_name, count(*) as cnt 
			FROM 
				`tblVideo` 
			WHERE 
				$addQry
	...
	function Analytics_Clinic_List ($args = '') {		
		...
		if($clinic1 != '') {
			if ($addQry)
			$addQry .= " AND ";
			$addQry .= " find_in_set('$clinic1',vd_clinic1)";
		}
		
		if($clinic2 != '') {
			if ($addQry)
			$addQry .= " AND ";
			$addQry .= " find_in_set('$clinic2',vd_clinic2)";
		}
		
		if($clinic3 != '') {
			if ($addQry)
			$addQry .= " AND ";
			$addQry .= " find_in_set('$clinic3',vd_clinic3)";
		}	
