...
//		echo $this -> conf_info['q_where'];
	...
	function getQuery ($type = '') {
		...
		if (!$list_dsp_none || $sc_type) {
			...
			$this -> setCounts ();
			$this -> getCounts ();
//			echo "===".$this->page_info['start_limit'];
			...
			for($j = 0 ;$j < $rstCnt1 ; $j++)
			{
				...
				$get_column_data = $this -> checkReplaceData ($data[$i],$column_name);
					...
					if($column_name == "cate1") {
						switch($get_column_data) {
							case 'A' : $get_column_data = '행위'; break;
							case 'B' : $get_column_data = '치료재료'; break;
							case 'C' : $get_column_data = '약제'; break;
							case 'D' : $get_column_data = '제증명수수료'; break;
							case 'E' : $get_column_data = '선택진료료'; break;


					}
				}
				
				if($column_name == "cate2") {
						switch($get_column_data) {
							case 'A-1' : $get_column_data = '상급병실료 차액'; break;
							case 'A-2' : $get_column_data = '검사료'; break;
							case 'A-3' : $get_column_data = '초음파 검사료'; break;
							case 'A-4' : $get_column_data = '초음파 영상료'; break;
							case 'A-5' : $get_column_data = '전산화단층영상진단(CT)'; break;
							case 'A-6' : $get_column_data = '자기공명영상진단료(MRI)'; break;
							case 'A-7' : $get_column_data = '이학요법료(물리치료료)'; break;
							case 'A-8' : $get_column_data = '처치 및 수술료 등'; break;
							case 'A-9' : $get_column_data = '치과 처치ㆍ수술료'; break;
							case 'A-10' : $get_column_data = '기타'; break;
							case 'B-1' : $get_column_data = '치료재료'; break;
							case 'B-2' : $get_column_data = '기타'; break;
							case 'C-1' : $get_column_data = '약제'; break;
							case 'D-1' : $get_column_data = '제증명수수료'; break;
							case 'E-1' : $get_column_data = '가격정보(%)'; break;
							case 'E-2' : $get_column_data = '가격정보(원)'; break;
				...
				if($column_name == "tor_rs_type") {
						switch($get_column_data) {
							case 'Y' : $get_column_data = '처리완료'; break;
							case 'N' : $get_column_data = '미처리'; break;
							case 'M' : $get_column_data = '처리중'; break;
