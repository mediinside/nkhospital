...
CLASS Reserve extends Dbconn
{
	...
	//예약 관리자 (온라인예약 진료일 설정)
	function Doctor_Schdule_Detail($dr_idx) {
		$qry = "
			SELECT so_apply_sdate,so_apply_edate FROM reserve_option where dr_idx = $dr_idx limit 0,1
		";
		$rst =  $this -> DB -> execSqlOneRow($qry);
		return $rst;
	}	
