...
class Naver{
	...
	function __construct($argv = array()) {
		...
		if($argv['CLASS_MODE'] != ""){
			$this->classMode = $argv['CLASS_MODE'];
		}
		$this->loadSession();
	...
	function login($options = array()){
		...
		if($this->loginMode == 'request' && (!$this->getConnectState()) || !$this->showLogout){
			if($this->classMode == "M") {
				echo '<a href="javascript:loginNaver();" class="sign-in-provide-naver">네이버 계정으로 로그인</a>';
			}else if($this->classMode == "v2") {
				echo '<a href="javascript:loginNaver();" class="btnNaver">네이버 로그인</a>';
			}else{
			//echo '<a href="javascript:loginNaver();" class="btnL btnNaver"><span>네이버 계정으로&nbsp;</span>로그인</a>';
			echo '<a href="javascript:loginNaver();" class="btn_sns_login btnNaverLogin">네이버 계정으로 로그인</a>';
			}
			echo '
			<script>
			function loginNaver(){
				var win = window.open(\''.NAVER_OAUTH_URL.'authorize?client_id='.$this->client_id.'&response_type=code&redirect_uri='.$this->returnURL.'&state='.$this->state.'\', \'네이버 계정으로 로그인\',\'width=320, height=480, toolbar=no, location=no\');
